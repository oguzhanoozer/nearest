...
class UserProfileService extends IUserProfileService with ErrorHelper {
  UserProfileService(GlobalKey<ScaffoldState> scaffoldState, BuildContext context) : super(scaffoldState, context);
  ...
  Future<void> deleteAccount() async {
    try {
      User? user = FirebaseAuthentication.instance.authCurrentUser();
      if (user != null) {
        await FirebaseCollectionRefInitialize.instance.usersCollectionReference.doc(user.uid).delete();
    ...
    try {
      ...
      if (user != null) {
        if (user.email.toString() == newEmail) {
          showSnackBar(scaffoldState, context, LocaleKeys.theEnterEmailAdressIsSameText.locale);
        } else {
