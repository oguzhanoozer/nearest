...
class ShopOwnerAddProductService extends IShopOwnerAddProductService with ErrorHelper {
  ShopOwnerAddProductService(GlobalKey<ScaffoldState> scaffoldState, BuildContext context) : super(scaffoldState, context);

  @override
  Future<void> addProduct(ProductDetailModel productData) async {
    try {
      await FirebaseCollectionRefInitialize.instance.productsCollectionReference.doc(productData.productId).set(productData.toMap());
    } catch (e) {
  ...
  Future<void> updateProduct(ProductDetailModel productData) async {
    try {
      await FirebaseCollectionRefInitialize.instance.productsCollectionReference.doc(productData.productId).update(productData.toMap());
    } catch (e) {
      showSnackBar(scaffoldState, context, LocaleKeys.updateProductError.locale);
    }
  }

  @override
  Future<List<String>> uploadImage(List<File> pickedFileList, String productId) async {
    int _currentIndex = 1;
    List<String> productImageUrlList = [];

    productImageUrlList = await Future.wait(pickedFileList.map((pickedFile) => uploadFile(File(pickedFile.path), _currentIndex++, productId)));

    return productImageUrlList;
  }

  @override
  Future<String> uploadFile(File _image, int imageIndex, String productId) async {
    String storageName = "${imageIndex} -> ${DateTime.now().millisecondsSinceEpoch.toString()}";
